...
PIXTYPE * RawImage::readStream(char const *filename,int *l,int * m,int  * n, int datatype)
{
	...
	*l=lx;*m=ly;*n=lz;
	...
	file.seekg(24L+*l**m*(212-20)*sizeof(float),ios::beg);//+512*512*345*sizeof(short)
	file.read((char *)buf,size);
	file.close();
	return buf;
...
void RawImage::writeImageNamev2(Raw &destImg, char *name)
{
	FILE *p = fopen(name, "wb");
	PIXTYPE *data = new PIXTYPE[destImg.size()];
	memcpy(data, destImg.getdata(), sizeof(PIXTYPE)*destImg.size());
